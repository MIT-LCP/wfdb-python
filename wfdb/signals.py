...
def getsegmentchannels(startseg, segrecordname, dirname, layoutfields, channels):
    ...
    if startseg == 0:  # Fixed layout signal. Channels for record are always same.
        ...
        if segrecordname != '~':
            sfields = rdheader(os.path.join(dirname, segrecordname))
...
def checkrecordfiles(recordname, pbdl, dldir):
    ...
    'downloadsamp'. This is a helper function for rdsamp. 
    ...
    if pbdl == 1:  
        dledfiles = downloadsamp(recordname, dldir)
...
def rdsamp(
    ...
    fields = rdheader(os.path.join(dirname, baserecordname))  
